...
from tools.pdf_extractor import extract_pdf, display_content
...
class State(TypedDict):
    ...
    rating: int 
    recommendation : str
    output: Annotated[list, "add messages"]
    extracted: str
...
def parse_pdf(state: State):
    formatted = state["extracted"].strip()
    print(formatted)
...
def show_result(state: State):
    ...
    print(message)
    return {"rating" : message.rate, "recommendation" : message.recom}
...
#         json.dump(json_object, f, indent=2, ensure_ascii=0 )


if __name__ == "__main__":
    extracted = extract_pdf("claude_daigan_resume.pdf")
    extracted_content = display_content(extracted)

    query = "english teacher"
    result = agent.invoke({"query": query, "extracted" : str(extracted_content)})
